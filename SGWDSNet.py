# -----------------------------------------------------------
# SGWDSNet.py - Cellpose风格梯度流场分割网络
# 默认: MSAS + SC2A + SGWDS + GASG + 梯度流预测
# -----------------------------------------------------------
import torch
...
class SGWDSNet(nn.Module):
    ...
    模块组合:
    - MSASBlock 作为编解码卷积块（内含 SC2A）
    - SGWDSBlock 上采样
    - GroupAdaptiveSkipGate (GASG) 跳跃连接融合
    - GradientFlowHead 梯度流场预测

    输出:
    - seg: 分割logits [B, n_classes, H, W]
    - flow: 梯度流场 [B, 2, H, W] (dy, dx)
    - (可选) cell_prob: 细胞概率 [B, 1, H, W]
    """
...
def compute_gradient_flow_from_mask(mask):
    ...
    参数:
        mask: [H, W] numpy数组, 二值mask (0=背景, 1=前景)

    返回:
        flow: [2, H, W] 梯度流场 (dy, dx)
    """
...
if __name__ == "__main__":
    print("=" * 60)
    print("SGWDSNet - Cellpose风格梯度流场分割网络")
    print("=" * 60)
    ...
    print(f"恢复的Mask形状: {recovered_mask.shape}")
